...
var d_leaderboard = [
	{ address: "0x28a4c3d86a9e971768043d90248a12c1ac67eb0b", tree: 0, pecan: 0, rank: 1 },
	{ address: "0x7f17931F856F7238FDe55746b958D63F6d082961", tree: 0, pecan: 0, rank: 2 },
	{ address: "0xF354685250D418BeBf4bc38A2616D3A23eeB1071", tree: 0, pecan: 0, rank: 3 },
	{ address: "0x20E7de28F3E50A801F31037E7ac549C00945a460", tree: 0, pecan: 0, rank: 4 },
	{ address: "0x4929822b91C0badE7AB055c33f35f8D043D57F3A", tree: 0, pecan: 0, rank: 5 }
];	
	...
	for(i = 1; i < 6; i++) {
		for(j = 0; j < 5; j++) {
			if(d_leaderboard[j].rank == i) {
				leaderboardArray[i].innerHTML = formatEthAdr(d_leaderboard[j].address) + "<br>" + numberWithSpaces(d_leaderboard[j].tree) + " MotherEGG Size <br>" + numberWithSpaces(d_leaderboard[j].pecan) + " Eggs <br>";
			}
		}
	}
}
...
claimedshareEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(storetxhash, result.transactionHash) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " claimed their share worth " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " BNB and got " + numberWithSpaces(result.args.pecan) + " Eggs.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
myContract.ClaimedShare({}, { fromBlock: launchBlock, toBlock: 'latest' }).get(function(error, result){
	if(!error){
		...
		for(i = 0; i < result.length; i++){
			if(checkHash(storetxhash, result[i].transactionHash) != 0) {
				eventlogdoc.innerHTML += "<br>[CLAIM] " + formatEthAdr(result[i].args.player) + " claimed their share worth " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " BNB and got " + result[i].args.pecan + " Eggs.";
				logboxscroll.scrollTop = logboxscroll.scrollHeight;
			}
		}
	}
...
wonroundEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(storetxhash, result.transactionHash) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " WINS ROUND " + result.args.round + " AND EARNS " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " BNB!";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
gavepecanEvent.watch(function(error, result){
	if(!error){
		...
		if(checkHash(storetxhash, result.transactionHash) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " gave " + numberWithSpaces(result.args.pecan) + " Eggs to Wonkers, and got " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " BNB in exchange!";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
withdrewbalanceEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(storetxhash, result.transactionHash) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " withdrew " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " BNB from their balance.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
paidthroneEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(storetxhash, result.transactionHash) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " paid tribute to the CRYPTOEGG! " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " BNB have been sent.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
boostedpotEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(storetxhash, result.transactionHash) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " makes a generous " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " BNB donation to the JackPot.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
